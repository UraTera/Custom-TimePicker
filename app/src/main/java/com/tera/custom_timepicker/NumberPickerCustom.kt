...
const val SHOW_ROW_5 = false // Показать 5 строк
const val INTERVAL_LONG_PRESS = 300 // Интервал обновления при длительном нажатии
...
class NumberPickerCustom(
    ...
    // Слушатель значения
    private var mListener: ((picker: NumberPickerCustom, value: Int) -> Unit)? = null
    ...
    // Атрибуты
    private var mMax = MAX_VALUE_HOUR
    ...
    var dividerHeight: Int = 0
        set(value) {
            field = value
            mPaintDivider.strokeWidth = value.toFloat()
        }

    var dividerOffset: Int = 0
        set(value) {
            field = value
            mDividerOffset = value
            setDividerOffset()
        }
    ...
    var fontFamily: Typeface? = mFontFamily
        set(value) {
            field = value
...
//        drawAxis(canvas)
    ...
    fun setOnChangeListener(listener: (picker: NumberPickerCustom, value: Int) -> Unit) {
        mListener = listener
    }
