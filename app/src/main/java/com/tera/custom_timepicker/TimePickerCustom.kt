...
import com.tera.custom_timepicker.databinding.TimePickerBinding
import java.util.Calendar

typealias OnValueChangeListener = (
    picker: TimePickerCustom, hour: Int, min: Int, sec: Int
) -> Unit
...
class TimePickerCustom(
    ...
    init {
        ...
        binding = TimePickerBinding.bind(this)

        val a = context.obtainStyledAttributes(attrs, R.styleable.TimePickerCustom)

        mDividerHeight =
            a.getDimensionPixelSize(R.styleable.TimePickerCustom_np_dividerHeight, DIVIDER_HEIGHT)
        mDividerColor = a.getColor(R.styleable.TimePickerCustom_np_dividerColor, DIVIDER_COLOR)
        mDividerOffset = a.getDimensionPixelSize(R.styleable.TimePickerCustom_np_dividerOffset, 0)

        val fading = a.getInt(R.styleable.TimePickerCustom_np_fadingExtent, FADING_EXTENT)
        mFadingExtent = fading / 10f

        mFontFamily = a.getFont(R.styleable.TimePickerCustom_np_fontFamily)
        mIntervalLongPress =
            a.getInt(R.styleable.TimePickerCustom_np_intervalLongPress, INTERVAL_LONG_PRESS)
        mMaxHour = a.getInt(R.styleable.TimePickerCustom_np_maxHours, MAX_VALUE_HOUR)

        mHintTextH = a.getString(R.styleable.TimePickerCustom_np_hintTextHour)
        mHintTextM = a.getString(R.styleable.TimePickerCustom_np_hintTextMin)
        mHintTextS = a.getString(R.styleable.TimePickerCustom_np_hintTextSec)
        mHintSize =
            a.getDimensionPixelSize(R.styleable.TimePickerCustom_np_hintTextSize, HINT_SIZE)
                .toFloat()
        mHintColor = a.getColor(R.styleable.TimePickerCustom_np_hintTextColor, TEXT_COLOR)

        mTextSize =
            a.getDimensionPixelSize(R.styleable.TimePickerCustom_np_textSize, TEXT_SIZE).toFloat()
        mTextSizeSel =
            a.getDimensionPixelSize(R.styleable.TimePickerCustom_np_textSizeSel, TEXT_SIZE_SEL)
                .toFloat()
        mTextColor =
            a.getColor(R.styleable.TimePickerCustom_np_textColor, TEXT_COLOR)
        mTextColorSel =
            a.getColor(R.styleable.TimePickerCustom_np_textColorSel, TEXT_COLOR_SEL)

        mShowRows5 = a.getBoolean(R.styleable.TimePickerCustom_np_showRows5, SHOW_ROW_5)
        mShowSec = a.getBoolean(R.styleable.TimePickerCustom_np_showSec, true)
        mShowTime = a.getBoolean(R.styleable.TimePickerCustom_np_showTime, true)
    ...
    private fun initParamText() = with(binding) {
        // Подсказка
        if (mHintTextH != null) {
    ...
    private fun initListener() {
        binding.pickerH.setOnChangeListener { _, value ->
            valueH = value
            if (mListener != null)
                mListener?.invoke(this, valueH, valueM, valueS)
        }
        binding.pickerM.setOnChangeListener { _, value ->
            valueM = value
            if (mListener != null)
                mListener?.invoke(this, valueH, valueM, valueS)
        }
        binding.pickerS.setOnChangeListener { _, value ->
            valueS = value
            if (mListener != null)
                mListener?.invoke(this, valueH, valueM, valueS)
        }
    ...
    var hour: Int
        get() = binding.pickerH.value
        set(value) {
            valueH = value
            binding.pickerH.value = value
        }

    var min: Int
        get() = binding.pickerM.value
        set(value) {
            valueM = value
            binding.pickerM.value = value
        }

    var sec: Int
        get() = binding.pickerS.value
        set(value) {
            valueS = value
            binding.pickerS.value = value
        }

    var scrollHour: Int = 0
        set(value) {
            field = value
            binding.pickerH.scroll = value
        }

    var scrollMin: Int = 0
        set(value) {
            field = value
            binding.pickerM.scroll = value
        }

    var scrollSec: Int = 0
        set(value) {
            field = value
            binding.pickerS.scroll = value
        }
